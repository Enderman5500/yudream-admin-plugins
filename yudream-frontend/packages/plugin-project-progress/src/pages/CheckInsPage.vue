    ...
    <section class="pp-grid">
      ...
      <section class="pp-panel">
        ...
        <FaResponsiveTable row-key="id" table-root-class="max-w-full overflow-x-auto rounded-lg" table-class="min-w-[900px]" border stripe column-visibility :columns="columns" :data="pagedRows" empty-text="暂无打卡记录">
          ...
          <template #cell-summary="{ row }"><strong>{{ row.original.summary || '无说明' }}</strong><div v-if="row.original.location" class="pp-table-sub">{{ row.original.location.address }}</div><div v-if="row.original.minecraft" class="pp-table-sub">{{ model.serverLabel(row.original.minecraft.serverId) }} · {{ model.minecraftScopeLabel(row.original.minecraft) }} · 有效在线 {{ model.minutes(row.original.minecraft.effectiveOnlineMillis) }}</div><div v-if="model.minecraftSubServerSummary(row.original.minecraft)" class="pp-table-sub">子服累计 {{ model.minecraftSubServerSummary(row.original.minecraft) }}</div><div v-if="row.original.files.length" class="pp-table-sub">{{ fileNames(row.original.files) }}</div></template>
            ...
            <FaCard class="w-full">
              <div class="flex flex-col gap-3">
                ...
                <div class="flex flex-col gap-1 text-sm">
                  ...
                  <div v-if="row.minecraft" class="flex gap-2"><span class="shrink-0 text-secondary-foreground/60">MC 在线</span><span class="break-all">{{ model.serverLabel(row.minecraft.serverId) }} · {{ model.minecraftScopeLabel(row.minecraft) }} · 有效在线 {{ model.minutes(row.minecraft.effectiveOnlineMillis) }}</span></div>
                  <div v-if="model.minecraftSubServerSummary(row.minecraft)" class="flex gap-2"><span class="shrink-0 text-secondary-foreground/60">MC 子服累计</span><span class="break-all">{{ model.minecraftSubServerSummary(row.minecraft) }}</span></div>
